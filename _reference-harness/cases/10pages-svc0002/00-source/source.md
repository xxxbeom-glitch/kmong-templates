# 10pages-svc0002 — Source

| 항목 | 값 |
|------|-----|
| caseId | `10pages-svc0002` |
| displayName | 10PAGE SAMPLE TAX |
| designCenterUrl | https://a2462129b.10pages.co.kr/ |
| demoMallUrl | https://a2462129b.10pages.co.kr/ |
| productCode | SVC0002 |
| collectedAt | 2026-07-09 |
| viewports | Desktop **1920** · Mobile **390** |
| track | **Track C** reference-harness |
| completeness | **browser-captured** (skin-zip 아님) |
| rightsType | demo-analysis |

## 수집 방법

- Playwright · 멀티페이지 pristine 미러 (`mirror-original.js`) + 캡처
- `site-clone-fidelity.md` 한 연속 작업

### 로컬에서 보기

**http://127.0.0.1:4173/**

```bash
cd _reference-harness
node scripts/preview-original.js 10pages-svc0002
# 재기동: node scripts/preview-original.js 10pages-svc0002 --force
```

## Track C 경계

- working skin / upload / 납품 **하지 않음**
