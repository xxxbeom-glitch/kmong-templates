...
## `scroll-reveal` 등장 단위 (프로젝트 기본값)

**별도 지시 없이 「스크롤 등장」 적용 시** 아래가 기본입니다. (`46-interaction-presets.mdc` · `45-interaction-patterns.mdc`)

| 구분 | 어떻게 뜨나 | 하면 안 되는 것 |
|------|-------------|----------------|
| **카드** (사진·흰 배경·캡션 한 프레임) | 카드 **통째로** (배경+글자 같이) | 배경만 있고 글자만 따로 뜨기 |
| **섹션 헤드** | 라벨 · 제목 **한 줄씩** · 본문 **한 문장씩** | — |
| **리스트** (카드 배경 없음) | 번호 · 제목 · 설명 **각각** | — |
| **hero · header · footer** | 안 씀 | — |

참고 구현: `wordpress/365-barun-dental/`
...
|--------|-----|-------|------------------------|
| **스크롤 등장** | `scroll-reveal` | 스크롤 시 블록이 살짝 올라오며 나타남 | **카드**는 통째로 · **헤드·본문**은 줄·문장 단위(기본값). hero·header 제외. **한 번만** 재생. |
