...
## 2026-07-16 — Figma·QA 규칙을 site/에 연결

| # | 결정 |
|---|------|
| 1 | `30`·`35`·`40`·`45`·`46`·`50`·`73` glob에 **`site/**` 추가** (WP glob 유지·동결) |
| 2 | 출력·배포 경로 기본 = **`site/` + Vercel** · WP ZIP은 재개 시만 |
| 3 | `site/` JS는 바닐라 허용 (`45`) · `60`/`10-static`은 활성=site 문구로 정리 |
| 4 | 새 규칙 파일 없음 · `70`~`72` 이사/삭제 없음 |
...
| # | 결정 |
|---|------|
| 1 | GEO는 별도 레인·파일 **없음** — `73` SEO/AEO와 **동일** |
| 2 | `73`에 엔티티 일관·직접 답(있을 때만)·1차 사실·Schema 힌트·날짜(해당 시) 보강 |
| 3 | 생성형 인용·추천 **비보장** · `llms.txt` 등 **필수 아님** |
...
| # | 결정 |
|---|------|
| 1 | SEO/AEO는 섹션 루프와 **별도** · **자동 실행 금지** |
| 2 | 콘텐츠 거의 확정 시 **알림 1회** · 미실행이면 납품 전 **알림 1회** (실행 아님) |
| 3 | 사용자 진행 요청 시에만 `73-seo-aeo.mdc` 읽고 작업 |
| 4 | 별도 content-seo 파일 **필수 아님** — 설계서·확정 콘텐츠·decision 우선 |
| 5 | 납품 QA: **완료 / 의도적 생략 / 미실행** 확인 (`50`) |
| 6 | 목적 = 이해 가능한 기반 · 순위·유입 **보장 아님** · 없는 사실 **임의 생성 금지** |
