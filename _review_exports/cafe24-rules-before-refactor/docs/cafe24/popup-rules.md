...
> SoT: `templates/cafe24_shop/` (read-only).  
> 적용: 스킨 분석 · working-copy · map · QA. **원본 스킨 파일을 이 문서만으로 수정하지 않는다.**
...
인벤토리에서는 관련 form·페이지를 `risk: "no-touch"` 우선으로 본다.
...
- `{$checkbox_today_open}`
- 동일 목적의 확인된 마크업  
  (예: SoT 내 `member/certification_layer.html`, `member/update_event.html`)
