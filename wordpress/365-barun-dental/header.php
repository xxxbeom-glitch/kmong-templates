<!DOCTYPE html>

<html <?php language_attributes(); ?>>

<head>

  <meta charset="<?php bloginfo('charset'); ?>">

  <meta name="viewport" content="width=device-width, initial-scale=1.0">

  <?php wp_head(); ?>

</head>

<body <?php body_class(); ?>>

<?php wp_body_open(); ?>



<header id="header" class="site-header">

  <div class="site-header__dim" aria-hidden="true" hidden></div>



  <div class="section-shell section-shell--gutter">

    <div class="site-header__inner">

      <a href="<?php echo esc_url(home_url('/')); ?>" class="site-header__logo">

        <img

          class="site-header__logo-img"

          src="<?php echo esc_url(barun_dental_asset_uri('logo-header')); ?>"

          alt="365 초록바른치과"

          width="229"

          height="49"

          decoding="async"

        >

      </a>



      <div class="site-header__right">

        <nav id="site-navigation" class="site-header__nav" aria-label="<?php esc_attr_e('Primary', '365-barun-dental'); ?>">

          <ul class="site-header__menu">

            <?php foreach (barun_dental_nav_menu() as $item) : ?>

              <?php

              $children = isset($item['children']) ? $item['children'] : array();

              $has_dropdown = count($children) > 1;

              $item_href = isset($item['href']) ? $item['href'] : '#';

              ?>

              <li class="site-header__item<?php echo $has_dropdown ? ' site-header__item--dropdown' : ''; ?>">

                <?php if ($has_dropdown) : ?>

                  <button

                    type="button"

                    class="site-header__trigger"

                    aria-expanded="false"

                    aria-haspopup="true"

                  >

                    <?php echo esc_html($item['label']); ?>

                  </button>

                  <div class="site-header__dropdown" hidden>

                    <ul class="site-header__submenu">

                      <?php foreach ($children as $child) : ?>

                        <li>

                          <a

                            href="<?php echo esc_url($child['href']); ?>"

                            class="site-header__sublink"

                          ><?php echo esc_html($child['label']); ?></a>

                        </li>

                      <?php endforeach; ?>

                    </ul>

                  </div>

                <?php else : ?>

                  <a href="<?php echo esc_url($item_href); ?>" class="site-header__link">

                    <?php echo esc_html($item['label']); ?>

                  </a>

                <?php endif; ?>

              </li>

            <?php endforeach; ?>

          </ul>

        </nav>



        <a href="<?php echo esc_url(barun_dental_consultation_url('list')); ?>" class="site-header__cta btn-slide-hover">

          <?php echo barun_dental_button_slide('상담·예약'); ?>

        </a>



        <button type="button" class="site-header__toggle" aria-expanded="false" aria-controls="site-navigation">

          <span class="site-header__toggle-bar" aria-hidden="true"></span>

          <span class="screen-reader-text">메뉴 열기</span>

        </button>

      </div>

    </div>

  </div>

</header>



<main class="site-main">

